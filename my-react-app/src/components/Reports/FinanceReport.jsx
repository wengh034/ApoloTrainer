...
const InformeMensual = ({ periodId }) => {
  ...
  useEffect(() => {
    const fetchCuotasData = async () => {
      ...
      try {
        const response = await fetch(`/api/filtered-payments-summary/${periodId}`);
    ...
    const fetchPendingPayments = async () => {
      setLoading(true);
      try {
        const response = await fetch(`/api/filtered-payments/${periodId}`);
        const result = await response.json();
        if (response.ok) {
  ...
  useEffect(() => {
    if (chartRef.current) { // Verifica que chartRef esté presente
      const chartInstance = echarts.init(chartRef.current); // Inicializa el gráfico
  ...
  return (
    <div>
      ...
      <SVGComponent src="./src/assets/download.svg" color="#ffc107" />
      </Button>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '20px' }}>
              <div style={{ flex: '1' }}>
                ...
                <div style={{ fontFamily:'Calibri', padding: '0 0 1em 0.5em' }}>
                  <div >
                  Estimado: {formatNumberWithDots(data.totalMonto)} Gs.
                  
                </div>
                <div >
                  Ingresos: {formatNumberWithDots(data.montoIngresado)} Gs.
                </div>
                <div >
                  Pendiente: {formatNumberWithDots(data.montoPendiente)} Gs.
                </div>
                <div>
