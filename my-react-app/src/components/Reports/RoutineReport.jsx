...
const RutinaDetail = ({ IdRutina, NombreRutina, Alumno }) => {
  ...
  const fetchRutinaData = async () => {
    setLoading(true);
    try {
      const responseDias = await fetch(`http://localhost:5000/api/rutinas/${IdRutina}/dias`);
      const dataDias = await responseDias.json();
      setDias(dataDias.data);

      const ejerciciosPromises = dataDias.data.map(async (dia) => {
        const responseEjercicios = await fetch(`http://localhost:5000/api/dias/${dia.IdDia}/ejercicios`);
  ...
  //   const element = document.getElementById('pdfContent');
  //   const options = {
  //     margin: [1, 0.8, 2, 0.8], // Ajustar márgenes para evitar cortes
  //     filename: `informe_cierre_${NombreRutina}.pdf`,
  //     image: { type: 'jpeg', quality: 0.98 },
  //     html2canvas: { scale: 3 },
  //     jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' },
  //     //pagebreak: { avoid: '.avoid-break' },  Clase para evitar saltos de página bruscos
  //   };
  ...
  //       pdf.setFontSize(30);
  //       pdf.text('Confidencial', pdf.internal.pageSize.getWidth() / 2, pdf.internal.pageSize.getHeight() / 2, {
  //         align: 'center',
  //         angle: -45,
  //       });
  //     }
  ...
  const handleDownloadPDF = () => {
    ...
    html2pdf()
      ...
      .then(async (pdf) => {
        const pageCount = pdf.internal.getNumberOfPages();
        const watermarkImage = './src/assets/Logo.png'; // Cambia a tu ruta correcta
  ...
  return (
    <div>
      <Button onClick={handleOpenModal} variant="btn btn-sm btn-secondary" style={{background: 'none'}}>
        <SVGComponent src="./src/assets/print.svg" color="#c3c5c9" />
      </Button>
