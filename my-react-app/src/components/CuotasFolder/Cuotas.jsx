...
const Cuotas = ({ selectedAlumno, cuotas, onStatusChange, onUpdateAlumno, handleDeleteClick }) => {
  ...
  useEffect(() => {
    const fetchCuotas = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/cuotas/${selectedAlumno.idAlumno}`);
  ...
  const handleDelete = async () => {
    if (selectedAlumno) {
      try {
        ...
        playSound('./src/assets/sounds/Pop-1.m4a');
  ...
  // const handleClose = () => {
  //   setShowModal(false);
  //   setAlumno(null); // Limpia el alumno seleccionado
  // };
  ...
  //  const StatusDesc = (status) => status === 5 ? 'Pagado' : status === 6 ? 'Pendiente' : 'Desconocido';
  ...
  const handleSave = async (e) => {
    ...
    try {
      const response = await fetch(`/api/cuotas/${selectedCuota.IdCuota}`, {
        method: 'PUT',
        headers: {
    ...
    <div className='card' style={{ width: '100%', color:'#c3c5c9', background:'#111827' }}>
      ...
      <div className='card-header' style={{minHeight:'147.25'}}>
        ...
        <div className="card-title" style={{display:'flex', fontWeight: 'bolder', marginBottom:'0', justifyContent:'space-between', padding:'10px'}}>
         ...
         <SVGComponent src="./src/assets/delete.svg" color="#c3c5c9"/>
          </Button>
         </span>
        </div>
        </div>
      ...
      <div className='card-body'style={{minHeight:'158px'}}>
        ...
        <div style={{display:'flex', marginBottom:'15px'}}>
          <span style={{marginRight:'10px'}}><SVGComponent src="./src/assets/person_single.svg" color="#717785" /></span>
          <span>{selectedAlumno ? formatNumberWithDots(selectedAlumno.CI) : ''}</span>
        </div>
        <div className='' style={{display:'flex', marginBottom:'15px'}}>
          <span style={{marginRight:'10px'}}><SVGComponent src="./src/assets/phone_number.svg" color="#717785" /></span>
          <span > {selectedAlumno ? selectedAlumno.Tel : ''}</span>
        </div>
        <div className='' style={{display:'flex', marginBottom:'15px'}}>
          <span style={{marginRight:'10px'}}><SVGComponent src="./src/assets/height.svg" color="#717785" /></span>
            {selectedAlumno ? selectedAlumno.Altura : ''}cm
        </div>

        <div style={{display:'flex', marginBottom:'15px'}}>
          <span style={{marginRight:'10px'}}><SVGComponent src="./src/assets/omniGender.svg" color="#717785" /></span>
          <span>{selectedAlumno ? SexInfo(selectedAlumno.Sexo) : ''}</span>
        </div>
        {cuotasActualizadas.map((cuota) => (
          <div key={cuota.IdCuota}>
              <div style={{display:'flex', marginBottom:'15px'}}>
                <span style={{marginRight:'10px'}}><SVGComponent src="./src/assets/calendar_today.svg" color="#717785" /></span>
