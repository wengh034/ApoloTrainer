...
const PendingPayments = () => {
  ...
  const handleConfirmResolve = async () => {
    if (selectedPayment) {
      try {
        const response = await fetch(`/api/pending-payments/${selectedPayment.IdPendPayment}/resolve`, {
          ...
          setPendingPayments(prevPayments =>
            prevPayments.map(payment =>
              payment.IdPendPayment === selectedPayment.IdPendPayment
                ? { ...payment, IdStates: 5, ResolveDate: currentDate } // Cambiar estado a 'Resuelto' y añadir ResolveDate
                : payment
            )
          );
  ...
  const filteredPayments = pendingPayments.filter(payment => {
    if (filter === 'resuelto') {
      return payment.IdStates === 5; // Solo resueltos
    } else if (filter === 'no resuelto') {
      return payment.IdStates === 6; // Solo no resueltos
    }
              ...
              <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', marginTop: '0.5em' }}>
                    <SVGComponent src="./src/assets/person_single.svg" color="#A1A1AA" />
              ...
              <div style={{ display: 'flex', alignItems: 'center', width: '100%', justifyContent: 'space-between', marginTop: '1em' }}>
                <div style={{ display: 'flex', alignItems: 'center' }}> <span style={{marginRight:'10px'}}> Periodo: </span><h5>{payment.Periodo}</h5></div>
                  ...
                  <div>{payment.IdStates === 5 && (
                    ...
                    <SVGComponent src="./src/assets/calendar_month.svg" color="#A1A1AA" />
